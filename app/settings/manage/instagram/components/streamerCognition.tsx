...
const CognitionStreamer: React.FC<InstagramStreamerProps> = ({
    userCurrentuserId,
  }) => {
  ...
  const handleParsedChunk = (parsedChunk: any) => {
    if (parsedChunk.starting) {
      setStatus('Starting...');
    } else if (parsedChunk.IgLength) {
      console.log('parsedChunk.IgLength: ', parsedChunk.IgLength)
      setStatus(`Retrieved ${parsedChunk.IgLength} media from your saved IG acount...`);
...
export default CognitionStreamer;
