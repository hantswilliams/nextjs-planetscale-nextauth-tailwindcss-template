...
export default async function({params}: {params: { slug: string }}) {
    ...
    const userImages2 = await fetch(`${process.env.NEXTAUTH_URL}/api/images/image/${params.slug}`, { cache: 'no-store' })
        .then((res) => res.json())
    
    const userCognitions2 = await fetch(`${process.env.NEXTAUTH_URL}/api/cognitions/image/${params.slug}`, { cache: 'no-store' })
        .then((res) => res.json())
